...
bool parseHex4(std::string_view text, std::size_t i, std::uint32_t& out) noexcept {
  if (i + 4 > text.size()) {
    return false;
  }
...
/// closing quote. Returns false on anything malformed rather than salvaging
/// what it can -- a half-decoded password is worse than no password.
    ...
    if (c == '\\') {
      ...
      switch (text[i]) {
        ...
        case 'u': {
          ...
          i += 5;
          // A high surrogate must be followed by its low half; anything else
          // is malformed rather than something to pass through.
          if (code >= 0xD800 && code <= 0xDBFF) {
            std::uint32_t low = 0;
            if (i + 1 >= text.size() || text[i] != '\\' || text[i + 1] != 'u' ||
                !parseHex4(text, i + 2, low) || low < 0xDC00 || low > 0xDFFF) {
              return false;
            }
            i += 6;
...
/// Advances `i` past one JSON value of any type. Nesting is bounded so a
/// deeply nested body cannot drive this into a stack overflow -- it is
/// iterative, and refuses beyond a depth no legitimate request reaches.
bool skipJsonValue(std::string_view text, std::size_t& i) {
  ...
  do {
    ...
    if (c == '{' || c == '[') {
      ...
      if (!parseJsonString(text, i, scratch)) {
        return false;
      }
    } else if (c == ',' || c == ':') {
      ++i;
...
std::string jsonFieldOf(std::string_view json, std::string_view field, bool* found) {
  if (found != nullptr) {
    *found = false;
  }
  std::size_t i = 0;
  ...
  for (;;) {
    ...
    if (json[i] == '}') {
      return {};  // walked the whole object without a match
    }
    if (json[i] == ',') {
      ++i;
      continue;
    ...
    if (key == field) {
      // Present but not a string counts as absent: silently coercing a number
      // or an object into text is how a caller ends up authenticating with the
      // literal string "null".
      if (json[i] != '"') {
        return {};
      }
      if (!parseJsonString(json, i, value)) {
        return {};
      }
      if (found != nullptr) {
        *found = true;
