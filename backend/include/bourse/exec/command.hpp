...
struct ServerContext {
  ...
  /// Null when the server was started without authentication, which is the
  /// default. The registry only enforces roles when this is present *and*
  /// enabled, so an unauthenticated local run behaves exactly as before.
...
struct CommandContext {
  ...
  /// Who is running this command. Set by the RESP connection after `AUTH`, or
  /// by the HTTP bearer middleware. Anonymous by default, which is only
  /// sufficient when the server is running without authentication.
  auth::Principal principal;

  /// Identifies the caller for login rate limiting -- a peer address, not an
...
class Command {
 public:
  ...
  /// Destructive or configuration-changing verbs -- FLUSHALL, CONFIG, USER.
  /// Separate from `isWrite()` because SET is a write that a trader should be
  /// able to run and FLUSHALL is not.
  [[nodiscard]] virtual bool isAdmin() const noexcept { return false; }

  /// Reachable without credentials even when auth is enforced. Only PING,
  /// AUTH, QUIT and HELLO say true: a client has to be able to greet the
  /// server and present credentials before it has any.
  [[nodiscard]] virtual bool isNoAuth() const noexcept { return false; }

  /// The minimum role required to run this verb, derived from the two flags
...
class LambdaCommand final : public Command {
 public:
  ...
  /// `is_admin` and `is_no_auth` are trailing and default to the safe value,
  /// so the forty-odd ordinary registrations stay unchanged and only the
  /// handful of privileged or pre-auth verbs have to say so.
  LambdaCommand(std::string name, int arity, bool is_write, std::string summary, Handler handler,
                bool is_admin = false, bool is_no_auth = false)
      : name_(std::move(name)),
