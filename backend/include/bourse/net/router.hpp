...
///
/// The request is mutable here and `const` in a handler, which is the whole
/// distinction between the two: middleware exists to annotate a request --
/// resolving a bearer token into a principal -- before anything acts on it.
using Middleware = std::function<void(HttpRequest&, HttpResponse&, const std::function<void()>& next)>;
