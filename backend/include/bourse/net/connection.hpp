...
class Connection : public std::enable_shared_from_this<Connection> {
 public:
  ...
  ///
  /// RESP is a session protocol: credentials are presented once and apply to
  /// every later command, unlike HTTP where each request carries its own. So
  /// the principal belongs to the connection, not to a request.
  ///
